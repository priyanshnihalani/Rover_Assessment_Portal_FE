...
export default function ExamUI() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
                <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center space-y-6">
                    ...
                    <h1 className="text-lg font-semibold text-slate-900">Session Expired</h1>

                    <p className="text-sm text-slate-500 leading-relaxed">
                        Your exam session is not valid or has expired. Please re-enter your email and paper code.
                    </p>
    ...
    const loadPaper = async () => {
        try {
            ...
            const initialTime = savedTime ? Number(savedTime) : result.data.timeLimit * 60;
    ...
    const submitExam = async () => {
        try {
            ...
            const payloadAnswers = questions.map((q) => {
                ...
                return {
                    questionId: q.id,
                    selectedOption: selectedOption !== undefined ? String.fromCharCode(65 + selectedOption) : '',
                };
            });
            const payload = {
                code,
            ...
            {loading ? (
                ...
                <>
                    {success === false && (
                        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
                            <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center space-y-6">
                                ...
                                <h1 className="text-lg font-semibold text-slate-900">Invalid Question Paper</h1>

                                <p className="text-sm text-slate-500 leading-relaxed">
                                    The question paper code you entered is not valid or has expired. Please check the code and try again.
                                </p>
                    ...
                    {success === true && Object.keys(result).length > 0 && (
                        <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
                            <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center space-y-6">
                                <h1 className="text-lg font-semibold text-slate-900 tracking-tight">Exam Result</h1>
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {result.score > 0 ? Math.round((result.score / result.total) * 100) : 0}% Score
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className=" bg-white text-slate-800">
                                ...
                                <div className="max-w-[1440px] mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex flex-row">
                                            <img src={"/logorth.png"} className="w-10 h-10" />
                                            <div className="ml-2">
                            ...
                            <div className="flex flex-col md:flex-row min-h-screen">
                                <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r p-4 sm:p-6 flex flex-col">
                                    <div>
                                        <h3 className="text-xs font-bold text-slate-400 uppercase mb-4">Questions</h3>
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            {questions.map((q, i) => {
                                                ...
                                                if (isActive) cls += ' border-orange-500 text-orange-600 ring-4 ring-orange-100';
                                                else if (isFlagged) cls += ' bg-amber-50 text-amber-600 border-amber-200';
                                                else if (isAnswered) cls += ' bg-slate-900 text-white';
                                                else cls += ' bg-slate-50 text-slate-400';

                                                return (
                                                    <div key={q.id} onClick={() => setCurrentIndex(i)} className={cls}>
                                ...
                                <main className="flex-1 p-4 sm:p-6 md:p-10">
                                    <div className="max-w-full sm:max-w-2xl md:max-w-3xl mx-auto space-y-6 sm:space-y-8 bg-white rounded-2xl border border-slate-100 p-4 sm:p-6 md:p-8">
                                        ...
                                        <div>
                                            ...
                                            <h2 className="text-xl font-medium leading-relaxed text-slate-900">{current.question}</h2>
                                        </div>
                                                    ...
                                                    >
                                                        ...
                                                        <span
                                                            className={`ml-4 text-sm ${
                                                                selected ? 'text-slate-900 font-medium' : 'text-slate-600'
                                                            }`}
                                                        >
                                                            {opt}
                                                        </span>
